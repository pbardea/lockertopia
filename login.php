...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  openConnect();
	...
	$salt = "js";
	$saltData = mysql_query("SELECT * FROM users WHERE username = '$username'");
	if ($saltData) {
    	 if (mysql_num_rows($saltData) == 0) {
        				header("Location: loginFailed.php?uname=false");
     	}	
     	else {
     	   while($saltRow = mysql_fetch_array( $saltData )) { 
 				$salt = $saltRow['salt']; 
 			} 
        ...
        closeConnect();
				...
				header("Location: loginFailed.php?uname=true");
				exit;
			}
     	}
	}
